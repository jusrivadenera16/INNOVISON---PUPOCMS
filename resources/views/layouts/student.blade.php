...
<body>
    ...
    @php
        ...
        $studentNeedsAccountType = $studentUser?->needsClinicAccountTypeSelection() ?? false;
        $studentPendingAdmission = $studentUser && $studentUser->hasPendingAdmissionReference();
        $studentClinicAudience = $studentUser?->clinicHealthFormAudience();
        if ($studentClinicAudience !== null) {
            $studentUsesDependentProfile = $studentClinicAudience === 'dependent';
            $studentUsesEmployeeHealthForm = $studentClinicAudience === 'employee';
        }
        ...
        if ($studentUser?->clinicHealthFormRoute()) {
            $studentHealthFormStartRoute = route($studentUser->clinicHealthFormRoute());
        }
        ...
        <div class="health-profile-prompt-card{{ $studentUsesDependentProfile ? ' is-dependent-prompt' : '' }}" style="background: #fff; border-radius: 24px; padding: 40px; max-width: 580px; width: 92%; text-align: center; box-shadow: 0 25px 80px rgba(0,0,0,0.4); margin: auto; position: relative; border-top: 2px solid #ffc107; border-bottom: 2px solid #ffc107;">
            ...
            <div class="health-profile-prepare">
                ...
                <div class="health-profile-prepare-grid">
                    @if($studentUsesDependentProfile || $studentNeedsAccountType)
                    <div>
                        <span>
    ...
    @if($showHealthFormModal && $studentNeedsAccountType)
        @include('student.partials.clinic_account_type_selector')
    @endif
